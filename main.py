...
# from models.maker_taker_model import MakerTakerModel, FeeModel
...
#             self.async_initialize_simulator(), 
...
#             # Start periodic update of orderbook display
#             if self.root:
...
#             # Show error in UI
#             if self.ui:
#                 self.ui.show_error(f"Failed to connect to orderbook: {e}")
...
#             # Set trade side (default to buy)
#             side = self.ui.side_var.get() if hasattr(self.ui, 'side_var') else "buy"
            
#             # Get additional parameters, with defaults if not in UI
#             volatility = float(self.ui.volatility_var.get()) if hasattr(self.ui, 'volatility_var') else 0.05
...
#         loop_thread = threading.Thread(target=self._run_async_loop, args=(loop,), daemon=True)
#         loop_thread.start()
